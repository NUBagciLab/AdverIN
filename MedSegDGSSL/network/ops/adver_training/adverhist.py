...
def interp1d(y, xi, nbins):
    ...
    select_left = torch.reshape(torch.index_select(y, dim=0, index=torch.flatten(index_round)), shape=index.shape)
    select_right = torch.reshape(torch.index_select(y, dim=0, index=torch.flatten(index_round_pluse_one)), shape=index.shape)
...
class AdverHist(nn.Module):
    ...
    """
    def __init__(self, prob:float=0.5, data_min:float=-1.0, data_max:float=1.0,
        ...
        self.params = nn.parameter.Parameter(torch.randn(self.num_control_point),
                                             requires_grad=True)
        self.axis = tuple([0])
    ...
    def forward(self, x):
        ...
        map_point =  torch.cumsum(torch.softmax(self.params, dim=0), dim=0)
        map_point = (map_point-torch.min(map_point))/(torch.max(map_point)-torch.min(map_point))
...
def adver_hist(cfg):
    model_cfg = cfg.MODEL
    adv = AdverHist(prob=model_cfg.ADVER_HIST.PROB,
