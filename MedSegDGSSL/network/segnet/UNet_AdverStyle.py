...
class Encoder(nn.Module):
    ...
    def forward(self, input, style_block:nn.Module=None):
        features = []
        input = self.input_block(input)
        for i in range(self.n_layers):
            features.append(input)
            if style_block is not None:
                #### Use correponding layer for adversarial training
                input = style_block((self.encoder_list[i](input)), i)
            else:
...
class UNet_EncDec(nn.Module):
    ...
    def forward(self, x: torch.Tensor, style_block:nn.Module=None) -> torch.Tensor:
        features = self.encoder(x, style_block)
...
if __name__ == '__main__':
    ...
    network = UNet_EncDec(spatial_dims=2,
